...
function extractRUCAndAmount(text, customPatterns = {}) {
  // Guarda el texto completo extraído para depuración y registro
  ...
  // Buscar número de comprobante (factura/boleta)
  let invoiceId = null;
  ...
  if (amountInWordsMatch) {
    ...
    if (wordAmount !== null) {
      ...
      console.log(
        `Monto en letras convertido a número: ${wordAmountWithCents}`
      );
      ...
      // O verificar que coincidan aproximadamente (pueden haber pequeñas diferencias por redondeo)
        ...
        console.log(
          `⚠️ Advertencia: Monto numérico (${amount}) no coincide con monto en letras (${wordAmountWithCents})`
        );
      }
    }
  }
  ...
  for (const pattern of invoiceNumberPatterns) {
    const match = text.match(pattern);
    if (match) {
      // Priorizar patrones robustos B/F + 3 dígitos + '-' + 6-8 dígitos
      ...
      if (invoiceNumber) {
        console.log(`🧾 Número de comprobante detectado: ${invoiceNumber}`);
      }
      break;
    }
  }
...
for (const pattern of invoiceNumberPatterns) {
  ...
  if (match) {
    ...
    if (invoiceId) {
      console.log(`🧾 Número de comprobante detectado: ${invoiceId}`);
    }
    break;
  }
}
...
return {
  ...
  location,
  purchaseDate,
  invoiceId, // número de comprobante
};
