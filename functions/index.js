...
// Importar módulos
const { verifyWhatsAppSignature, sendWhatsAppMessage, normalizePhoneNumber } = require('./src/whatsapp/messaging');
const { processImageMessage, processTextMessage } = require('./src/whatsapp/processMessages');
...
const WHATSAPP_PHONE_NUMBER_ID = process.env.WHATSAPP_PHONE_NUMBER_ID || '639581415900129'; // Actualizado según el webhook
const WHATSAPP_VERIFY_TOKEN = process.env.WHATSAPP_VERIFY_TOKEN || '38f7d5a1-b65c-4e9d-9f2d-ea9c21b7ca56';
const WHATSAPP_API_TOKEN = process.env.WHATSAPP_API_TOKEN || 'EAAJoZCiJisnoBO1f9v4mv7FRkD5kOJyxF2eNwNLyAuFzRYURV2Jeau3p2ZBd0bbQleo1jbXM4fYiKZANk0GTIwyRaIs1qQ1XJ6Ab1qhIttcLntKq7WUSXZAVS9WDJ2XotSQunELvsoz8xth9ymrCwlEPMxhOiOAMVuPGgLuqaw4jkZC8SvOEVa7fbDuNw2AZDZD';
const WHATSAPP_APP_SECRET = process.env.WHATSAPP_APP_SECRET || '09353d1301e356b0cdcba78d2a9c7639';
...
whatsappApiApp.post('/', verifySignatureMiddleware, async (req, res) => {
  try {
    ...
    console.log('Webhook de WhatsApp recibido:', JSON.stringify(body, null, 2).substring(0, 500) + '...');
          ...
          for (const message of value.messages || []) {
            try {
              ...
              if (message.type === 'image') {
                ...
                processImageMessage(message, user, WHATSAPP_PHONE_NUMBER_ID, WHATSAPP_API_TOKEN)
                  .catch(err => console.error('Error procesando imagen:', err));
              } else if (message.type === 'text') {
                // Si es un texto, procesamos comandos o consultas
                processTextMessage(message, user, WHATSAPP_PHONE_NUMBER_ID, WHATSAPP_API_TOKEN)
                ...
                sendWhatsAppMessage(
                  ...
                  WHATSAPP_PHONE_NUMBER_ID,
                  WHATSAPP_API_TOKEN
                ).catch(err => console.error('Error enviando mensaje genérico:', err));
              }
            } catch (messageError) {
          ...
          for (const message of value.messages || []) {
            try {
              ...
              const user = await Promise.race([
                userPromise,
                new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout buscando usuario')), 5000))
              ]);
              ...
              switch (message.type) {
                case 'image':
                  ...
                  processImageMessage(message, user, phoneNumberId)
                    .catch(error => {
                ...
                case 'text':
                  ...
                  // Si es un texto, procesamos comandos o consultas
                  processTextMessage(message, user, phoneNumberId)
                    .catch(error => {
